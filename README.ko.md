...
- DMG·PKG·압축 해제 직후 실행이 막히는 파일을 열 때
- "손상되었기 때문에 열 수 없습니다"가 아니라 **차단 속성 때문**인 경우를 빠르게 해소할 때
- 서명이 깨진 앱은 **경고를 보여준 뒤에만** 실행할 때

## 사용 방법

1. `.app` / `.dmg` / `.pkg`를 열어줘 창에 드래그하거나 `⌘O`로 파일을 선택합니다.
2. 파이프라인이 끝까지 자동 실행됩니다: 확인 → 제거 → 검증 → 서명 → 평가.
   - `.dmg`: 격리 해제 → 마운트 → 내부 `.app` 추출 (설치 DMG는 `.pkg` 폴백) → 앱/패키지 파이프라인 연결
   - `.pkg`: 격리 해제 → `pkgutil`/`spctl -t install` 검사 → 설치기 열기
3. 검증 후에는 **서명하시겠습니까?**가 뜹니다. 서명하면 본인 개발자 신원이 앱에 부여되고, 건너뛰어도 검사는 계속됩니다.
4. 결과에 따라 **실행 버튼**이 활성화되면 직접 눌러 실행합니다. 자동 실행은 없습니다.

**첫 실행에는 도움말 시트가 한 번만** 열립니다 (7섹션: 왜 필요한가요 · 사용 절차 · 파일 형식별 처리 · 준비됨과 경고 · 경고의 의미 · 하지 않는 일 · 자주 나오는 문제). 닫으면 다음부터 자동으로 뜨지 않으며, `⌘/` · 드롭존 링크 · 하단바로 언제든 다시 열 수 있습니다.
...
| `경고 — 변조 의심` + 체크박스 | spctl 거부·중첩 속성 손상이어도 차단 속성 제거까지 끝난 상태 | 카드 확인 후 체크박스 선택 → 실행 (터미널 위임으로 여세요) |
...
- `.pkg`는 설치기 열기까지만 지원하며 재서명하지 않습니다
...
4. 그 다음 열어줘가 **도움말 시트를 한 번만** 열어 각 상태의 뜻을 알려줍니다
...
./build_and_run.sh package macos # → dist/AGO-<버전>-macos.dmg (VERSION=0.3.1 지정 가능)
```
